...
def load_student_management_page(frame, data_manager, callbacks):
    ...
    add_student_button = tkinter.Button(
        frame,
        text="Add Student",
        command=lambda: add_student(data_manager, add_student_inputs, student_list),
    )
    ...
    student_list_label.place(
        relx=0.5, rely=0.175, relwidth=0.1, relheight=0.05, anchor=tkinter.CENTER
    )
    student_list.place(
        relx=0.5, rely=0.4, relwidth=0.2, relheight=0.4, anchor=tkinter.CENTER
    )

    edit_student_button.place(
        relx=0.4, rely=0.65, relwidth=0.1, relheight=0.05, anchor=tkinter.CENTER
    )
    remove_student_button.place(
        relx=0.6, rely=0.65, relwidth=0.1, relheight=0.05, anchor=tkinter.CENTER
    )

    add_student_first_name_label.place(
        relx=0.45, rely=0.75, relwidth=0.1, relheight=0.05, anchor=tkinter.CENTER
    )
    add_student_last_name_label.place(
        relx=0.55, rely=0.75, relwidth=0.1, relheight=0.05, anchor=tkinter.CENTER
    )

    add_student_first_name_entry.place(
        relx=0.45, rely=0.8, relwidth=0.1, relheight=0.05, anchor=tkinter.CENTER
    )
    add_student_last_name_entry.place(
        relx=0.55, rely=0.8, relwidth=0.1, relheight=0.05, anchor=tkinter.CENTER
    )

    add_student_button.place(
        relx=0.5, rely=0.9, relwidth=0.1, relheight=0.05, anchor=tkinter.CENTER
    )
...
def edit_student(callbacks, data_manager, student_id):
    ...
    classes_options = [
        f"{classes.name} (ID: {classes.id})" for classes in data_manager.classes
    ]
    grades_options = [
        "A+",
        "A",
    ...
    classes_option_menu_for_classes.place(
        relx=0.5, rely=0.3, relwidth=0.1, relheight=0.05, anchor=tkinter.CENTER
    )
    classes_option_menu_for_grades.place(
        relx=0.45, rely=0.45, relwidth=0.1, relheight=0.05, anchor=tkinter.CENTER
    )
    grades_option_menu.place(
        relx=0.55, rely=0.45, relwidth=0.1, relheight=0.05, anchor=tkinter.CENTER
    )

    add_class_button.place(
        relx=0.45, rely=0.35, relwidth=0.1, relheight=0.05, anchor=tkinter.CENTER
    )
    remove_class_button.place(
        relx=0.55, rely=0.35, relwidth=0.1, relheight=0.05, anchor=tkinter.CENTER
    )
    add_grades_button.place(
        relx=0.5, rely=0.5, relwidth=0.1, relheight=0.05, anchor=tkinter.CENTER
    )

    edit_student_frame.place(
        relx=0.5, rely=0.5, relwidth=1, relheight=1, anchor=tkinter.CENTER
    )
                ...
                if any(c["class_id"] == class_id for c in student.classes):
                    ...
                    if add_or_remove == "add":
                        ...
                        messagebox.showwarning(
                            "Class Exists",
                            f"{student.full_name()} is already enrolled in {class_name}.",
                        )
                        return
                    else:
                        ...
                        if confirm:
                            student.classes = [
                                c for c in student.classes if c["class_id"] != class_id
                            ]
                    ...
                    if add_or_remove == "remove":
                        ...
                        messagebox.showwarning(
                            "Class Not Found",
                            f"{student.full_name()} is not enrolled in {class_name}.",
                        )
                        return
                    else:
...
def change_grades(
    data_manager, student_id, student_list, classes_option_menu, grades_option_menu
):
            ...
            if student.id == student_id:
                ...
                for classes in student.classes:
                    ...
                    if classes["class_id"] == class_id:
                        ...
                        confirm = messagebox.askokcancel(
                            "Grade Updated",
                            f"Updated grade for {selected_class.split(' (ID:')[0]} to {selected_grade} for {student.full_name()}.",
                        )
                        if not confirm:
                            return
                ...
                if found == 0:
                    ...
                    messagebox.showwarning(
                        "Class Not Found",
                        f"{student.full_name()} is not enrolled in {selected_class.split(' (ID:')[0]}.",
                    )
...
def remove_student(data_manager, student_id, student_list):
    ...
    student_to_remove = next(
        (student for student in data_manager.students if student.id == student_id), None
    )
